...
def GetFallAmount(z,c_num):
    same_z = []
    above_z = []
    for x in range(len(cube_map[z])):
        for y in range(len(cube_map[z][x])):
            current_c = cube_map[z][x][y]
            if current_c != 0 and current_c != c_num and current_c not in same_z:
                same_z.append(current_c)
    for z_range in range(len(cube_map)-z-1):
        z_index = z + z_range + 1
        for x in range(len(cube_map[z])):
            for y in range(len(cube_map[z][x])):
                current_c = cube_map[z_index][x][y]
                if current_c != 0 and current_c != c_num and current_c not in same_z and current_c not in above_z:
                    above_z.append(current_c)
    return len(above_z)
    ...
    if axis == "z":
        if z+length != len(cube_map):
            if cube_map[z+length][x][y] != 0:
                if not SafeCheck(cube_map[z+length][x][y], c_num):
                    falling_amount += GetFallAmount(z,c_num)
    else:
        if z+1 != len(cube_map):
            h_cubes = []
            safe = True
