...
for line in record_list:
    ...
    if "Guard" in text:
        ...
        if current_guard not in guard_record:
            guard_record[current_guard] = [0] * 60
...
for p in guard_record:
    if max(guard_record[p]) > most_sleep_amount:
        most_sleep_amount = max(guard_record[p])
        most_sleep_id = p
        
record = guard_record[most_sleep_id]
max_minute = record.index(most_sleep_amount)
